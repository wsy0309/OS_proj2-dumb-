...
/*************************************************************************
checktlb : tlb에 addr의 값의 유무를 체크 & 업데이트
 - valid : tlb에 해당 index의 data확인 (유무)
    -> L1Index 확인(일치, 불일치)
        -> 불일치의 경우 dirty체크(tlb update check)
 - hit : tlb에 해당 index, L1Index의 data가 존재하는 경우
 - cold miss : tlb에 해당 index의 data가 존재하지 않는
   -> valid = 1로 set
 - conflict miss : tlb에 해당 index의 data가 존재하나 L1Index가 다를 경우
*************************************************************************/
